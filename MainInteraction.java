...
public class MainInteraction {
	...
	public void addUser(User user) {
		...
		this.user[counterUser++] = user;
		
	}
	
	public void importUsers(MainInteraction mi1) {
		
		Iterator itUsers = mi1.iteratorUsers();
		
		while(itUsers.hasNext()) {
			
			User user = itUsers.nextUser();
	...
	private boolean foundUser(User otherUser) {
		
		boolean found = false;
		
		for(int i = 0; i < counterUser && !found; i++) {
			
			if(user[i].equals(otherUser))
				found = true;
	...
	/** Getter for the method "isDateValid":
	 * 
	 * @param date
	 * @return
	 */
